...
beforeAll(/* async */ state => {return (fn => {
  ...
  state.httpsServer = (yield TestServer.createHTTPS(assetsPath, httpsPort));
...
afterAll(/* async */({server, httpsServer}) => {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
...
beforeEach(/* async */({server, httpsServer}) => {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
...
describe('Browser', function() {
  ...
  beforeEach(/* async */(state, test) => {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
  ...
  return new Promise((resolve, reject) => {
    ...
    return step('next');
      ...
      state.context = (yield state.browser.createIncognitoBrowserContext());
      state.page = (yield state.context.newPage());
    });});
...
if (process.env.COVERAGE) {
  describe('COVERAGE', function() {
    ...
    for (const method of coverage.keys()) {
      (disabled.has(method) ? xit : it)(`public api '${method}' should be called`, /* async */({page, server}) => {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
