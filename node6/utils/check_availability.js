...
/* async */ function checkOmahaProxyAvailability() {return (fn => {
  ...
  const lastchanged = ((yield Promise.all([
 ...
 */
/* async */ function checkRangeAvailability(fromRevision, toRevision) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
  ...
  for (let revision = fromRevision; revision !== toRevision; revision += inc)
    (yield checkAndDrawRevisionAvailability(table, '', revision));
});}
 ...
 */
/* async */ function checkAndDrawRevisionAvailability(table, name, revision) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
  ...
  const availability = (yield Promise.all(promises));
