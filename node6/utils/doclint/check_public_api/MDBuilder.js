...
class MDOutline {
   ...
   * @return {!MDOutline}
   */
  static /* async */ create(page, text) {return (fn => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const {classes, errors} = (yield page.evaluate(() => {
      const classes = [];
      let currentClass = {};
      let member = {};
      const errors = [];
 ...
 */
module.exports = /* async */ function(page, sources) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
  ...
  for (const source of sources) {
    const outline = (yield MDOutline.create(page, source.text()));
