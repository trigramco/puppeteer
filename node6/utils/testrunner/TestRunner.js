...
class TestPass {
  ...
  /* async */ _runSuite(workerId, suitesStack, state) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
    ...
    if (!this._workerDistribution.hasValue(currentSuite, workerId))
      return;
    (yield this._runHook(workerId, currentSuite, 'beforeAll', state));
    for (const child of currentSuite.children) {
      ...
      if (child instanceof Test) {
        for (let i = 0; i < suitesStack.length; i++)
          (yield this._runHook(workerId, suitesStack[i], 'beforeEach', state, child));
        (yield this._runTest(workerId, child, state));
        for (let i = suitesStack.length - 1; i >= 0; i--)
          (yield this._runHook(workerId, suitesStack[i], 'afterEach', state, child));
      } else {
        suitesStack.push(child);
        (yield this._runSuite(workerId, suitesStack, state));
        suitesStack.pop();
      }
    }
    (yield this._runHook(workerId, currentSuite, 'afterAll', state));
  });}

  /* async */ _runTest(workerId, test, state) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
    ...
    const error = (yield test._userCallback.run(state, test));
  ...
  /* async */ _runHook(workerId, suite, hookName, ...args) {return (fn => {
  const gen = fn.call(this);
  return new Promise((resolve, reject) => {
    function step(key, arg) {
      let info, value;
    ...
    const error = (yield hook.run(...args));
...
class TestRunner extends EventEmitter {
  ...
  return new Promise((resolve, reject) => {
    ...
    const termination = (yield pass.run());
    if (termination)
